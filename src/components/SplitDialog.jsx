...
function SplitDialog({ item, currentMember, allMembers, onConfirm, onClose }) {
  ...
  // Reset quantities when the dialog is opened for a different item/member
  useEffect(() => {
    setQuantities({ [currentMember?.id]: 1 });
  }, [item?.item_name, item?.price, currentMember?.id]);
  ...
  return (
    ...
    >
      <DialogPrimitive.Portal>
        <DialogPrimitive.Overlay className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[60] data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0" />
        <DialogPrimitive.Content
          onOpenAutoFocus={(e) => e.preventDefault()}
          className="fixed left-1/2 bottom-0 sm:bottom-auto sm:top-1/2 -translate-x-1/2 sm:-translate-y-1/2 w-full max-w-md bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl max-h-[85vh] flex flex-col z-[61] data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=open]:slide-in-from-bottom-4"
        >
          {/* Header */}
          <div className="px-5 pt-5 pb-3 border-b border-slate-100">
            <div className="flex items-center justify-between mb-3">
              <DialogPrimitive.Title className="text-lg font-bold text-slate-800">
                分攤項目
