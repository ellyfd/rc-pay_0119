  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      **重要：訂單結構說明**
      Uber Eats 團購訂單的版面配置：
      1. 頂部：店家名稱
      2. 主要內容：各個成員的訂單區塊，**每個成員一個區塊**
      3. **每個成員區塊的結構**（這是關鍵）：
         - 左側有圓形頭像/圖標
         - 右側是成員名字（可能帶 "(you)" 標記或 "Adding items" 文字）
         - **成員名字正下方緊接著該成員訂購的所有商品**
         - 每個商品有：數字標記（如 1、2、3）+ 商品名稱 + 價格
      4. 可能有 "Others in your group" 分隔線，下方繼續是其他成員區塊
      5. 底部：費用明細（外送費、服務費等）

      **關鍵規則：成員名字下方的所有商品（直到下一個成員名字出現）都屬於該成員**
