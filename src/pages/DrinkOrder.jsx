...
**訂單結構重要說明：**
此為 Uber Eats 團購訂單，訂單會按成員分組顯示：
- 每個成員會有一個標題區塊（例如："Alan", "SIXIAN", "Karen" 等）
- 標題後面可能會標註 "Creator"（開團者）、"(you)"（你本人）、"Adding items"（正在加入項目）等
- 每個成員標題下方會列出該成員訂購的所有項目
- 每個項目左側都有數字標記（1、2、3...）

**訂購人名稱辨識（非常重要）：**
1. 成員名字顯示在群組標題位置（大字體）
2. 標題後可能有這些標記（請忽略）：
   - "Creator" 或 "- 2 items" 等項目數量
   - "(you)" 或 "(您)" 代表訂單支付者
   - "Adding items" 或其他狀態
3. **請完整提取每個群組標題的成員名字**，並移除上述標記
4. 例如：看到 "SIXIAN Creator - 2 items"，請提取 "SIXIAN"
5. 例如：看到 "RC (You) Mark yourself as ready"，請提取 "RC"

**訂單項目的辨識方式：**
- 在每個成員名字下方，會有該成員的訂購項目列表
- 每個產品的左側會有一個數字標記（例如：框框內的數字 1、2、3 等）
- 產品名稱在數字標記的右側
- 金額顯示在產品名稱的水平右側位置
- 請忽略溫度、備註等細節，只提取產品名稱和價格
- **確保每個項目都對應到正確的成員群組**
...
**請提取以下資訊：**
1. 店家名稱
2. 識別所有成員群組（包含標記為 Creator 的成員）
3. 每個成員群組下的所有訂購項目（產品名稱和價格）
...
- items: 訂單項目陣列，每個項目包含：
  - member_name: 訂購人名字（已完整移除 "Creator"、"(you)"、"(您)"、項目數量等標記）
  - item_name: 飲料/產品名稱
  - price: 價格（數字，不含貨幣符號）

**重要提醒：請仔細識別每個成員群組，特別是標記為 "Creator" 的開團者，確保他們的訂單項目也被完整提取。**`,
